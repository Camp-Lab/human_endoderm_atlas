...
tissue.cols <- setNames(c("#F9C961","#faae65", "#c89721", "#8f6c19", "#227539", "#9288eb", "#5b4e9f", "#9d0142", "#f392be", "#fb75b1", "#a84b99", "#9d0142", "#9F3778", "#7960A9"),
...
beach.col <- c("#ffffcc", "#a1dab4", "#41b6c4", "#2c7fb8", "#253494")
darkBlue2Red <- c("#053061", "#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#F7F7F7", "#FDDBC7", "#F4A582", "#D6604D", "#B2182B", "#67001F")
darkBlue <- rev(c("#053061", "#2166AC", "#4393C3", "#92C5DE", "#D1E5F0", "#F7F7F7"))

beach.col.heatmap <- colorRampPalette(beach.col)(50) 
black2red <- colorRampPalette(c("#404040", "#bababa", "#ffffff", "#f4a582", "#ca0020"))(50)
age.cols <- c("#bdbdbd", "#252525")
epi.ct.cols <- setNames(c("#27AE60", "#F1C40F", "#2980B9", "#8E44AD",
                          "#C0392B", "#16A085", "#D35400", "#C51B8A","#D7BDE2"),
                        c("Enteroendocrine", "Enterocyte_precursor", "Goblet", "Stem_cell",
                          "Enterocyte", "M_cell", "Tuft", "SPIB-/GP2+/MUC1_M-cell", "Paneth"))
stage.cols <- setNames(c("#f1c40f","#1abc9c","#3498db", "#2980B9", "#A569BD", "#C39BD3"),
                       c("thio", "developing-duo", "adult-duo",  "adult-ile", "enteroid-IF", "enteroid-SB"))
treatment.cols <- setNames(c("#f4ed91", "#2b83ba", "#d7191c", "#fdae61", "#abdda4"),
                           c("0E-0N", "0E-100N", "100E-0N", "100E-1N", "1E-100N"))
prettyrainbow <- c("#8966A9","#3FA4D9","#3EB8B4","#B2C224","#F8CB31","#F6A22B","#EC6325","#DC3838")
#tissue.group.cols <- setNames(c("#fcb163","#4da7b0","#5d4ea2","#9d0142","#2471A3","#a9489a","#1b7635", "#d9d9d9", "#9d0142"), c("Lung", "Liver", "Stomach"i"Duodenum_SI", "Other_SI_region", "Colon", "Esophagus", "UD", "SI"))
col.vec <- c("#F9E79F","#F4D03F", "#F5B041", "#DC7633","#1b7635","#BB8FCE","#A569BD","#6C3483","#EC7063","#EC7063","#9d0142","#2471A3","#3498DB","#02818a", "#17A589", "#303030", "#fa9fb5", "#dd3497")
tissue.vec <- c("Lung-tracheal-epi","Lung-airway-trachea", "Lung-airway", "Lung-distal", "Esophagus", "Stomach","Stomach-corpus", "Stomach-antrum", "Small-intestine", "Proximal-small-intestine", "Duodenum", "Jejunum", "Ileum", "Colon", "Liver", "DE", "HIO", "tHIO") 
tissue.cols <- setNames(col.vec,tissue.vec)
cell.type.cols <- setNames(c("#8E44AD", "#DD3497", "#08519C", "#A50F15", "#006D2C","#FACB12", "#D35400", "#91C2E1"),c("Endothelial", "Neural", "Immune", "Epithelial", "Mesenchymal", "Hepatocyte", "Erythroid", "Erythroid_and_immune"))
col.vec <- c("#A4D371", "#A64A97", "#CA6778", "#DCCB7C", "#5E4FA2", "#3C7AB6", "#138D75", "#80B1D3", "#4DAA99", "#52BE80", 
             "#756BB1", "#322A84", "#F8C471", "#7BCAA4", "#9E0142")
ct.vec <- c("Proliferative",        "NRG1+_subepithelial",  "CCL19+",              "GDF10+",              
            "PITX1+_SM",            "Pericyte",             "CD81_high",            "GPX3+_villus-core",   
            "LXN+_intermediate",    "ICC",                  "KCNN3+",               "VSM",                 
            "CRABP1+",              "SFRP1+_proliferative", "Mesothelial-cell")
duo.mes.col <- setNames(col.vec, ct.vec)
...
e.n.ct.cols <- setNames(c("#f9e79f", "#a9dfbf", "#4daa99",
                          "#7DCEA0","#756BB1","#138D75", "#C39BD3",
                          "#08519c", "#3c7ab6","#8E44AD",
                          "#bb8fce", "#756bb1","#f0b27a",
                          "#85c1e9", "#4da7b0",
                          "#e6f2df","#b5cae5","#27AE60",
                          "#758ab1","#ca6778", "#b06754"),
                        c("Proliferative mesenchymal", "CXCL14+",  "ANO1+/HAND1+",             
                          "Intermediate 2",            "PRRX1 type 1", "Intermediate 1", "PRRX1 type 2",          
                          "KRT4+ basal type 2",      "SPRR2F+ basal", "Intestinal stem cell",     
                          "Proliferative epithelial",  "Stomach-secretory-like 2",  "Ciliated-like",            
...
#' @export
plotFeature2 <- function(coor, values, label.only=FALSE, point.order= NULL, knn.pairs=NULL, emphasize=NULL, 
                         nCols=NULL, gCols=NULL, group.order=NULL, zeroAsGray=T, add.legend=F, add.label=F, 
                         label.cex=1, legend.cex=1, legend.pos="topright", add.cluster=FALSE, cl.vec=NULL, 
                         with.outline=T, outline.col="#303030", pch=ifelse(with.outline, 21, 16), lwd=0.1, ...){
  if(label.only){
  ...
  if(is.null(knn.pairs)){
    if(is.null(emphasize)){
      ...
      if(!with.outline){
        plot(point.coor, col=cellColor, pch=pch, cex.main=5, xaxt="n", yaxt="n", xlab="", ylab="", bty="n", ...)
      }else{
        plot(point.coor, col=outline.col, bg=cellColor, pch=pch, cex.main=5, xaxt="n", yaxt="n", xlab="", ylab="", bty="n", lwd=lwd, ...)
      }
      
    }else{
      if(!with.outline){
        plot(coor, col="#bdbdbd", pch=pch, cex.main=5, xaxt="n", yaxt="n", xlab="", ylab="", bty="n",  ...)
        if(is.null(point.order)){
          points(coor[emphasize,], col=cellColor[emphasize], pch=pch, ...)
        }else if(point.order=="sorted"){
          idx <- order(values[emphasize])
          points(coor[emphasize,][idx,], col=cellColor[emphasize][idx], pch=pch, ...)
        }else if(point.order=="random"){
          idx <- sample(seq(length(values[emphasize])))
          points(coor[emphasize,][idx,], col=cellColor[emphasize][idx], pch=pch, ...)
        }
      }else{
        plot(coor, col=outline.col, bg="#bdbdbd", pch=pch, cex.main=5, xaxt="n", yaxt="n", xlab="", ylab="", bty="n",  ...)
        if(is.null(point.order)){
          points(coor[emphasize,], col=outline.col, bg=cellColor[emphasize], pch=pch, lwd=lwd, ...)
        }else if(point.order=="sorted"){
          idx <- order(values[emphasize])
          points(coor[emphasize,][idx,], col=outline.col, bg=cellColor[emphasize][idx], pch=pch, lwd=lwd, ...)
        }else if(point.order=="random"){
          idx <- sample(seq(length(values[emphasize])))
          points(coor[emphasize,][idx,], col=outline.col, bg=cellColor[emphasize][idx], pch=pch, lwd=lwd, ...)
        }
    ...
    if(!with.outline){
      if(is.null(emphasize)){
        points(coor, col=cellColor, pch=pch, ...)
      }else{
        points(coor, col="#bdbdbd", pch=pch, ...)
        points(coor[emphasize,], col=cellColor[emphasize], pch=pch, ...)
      }
    }else{
      if(is.null(emphasize)){
        points(coor, col=outline.col, bg=cellColor, pch=pch, lwd=lwd, ...)
      }else{
        points(coor, col=outline.col, bg="#bdbdbd", pch=pch, lwd=lwd, ...)
        points(coor[emphasize,], col=outline.col, bg=cellColor[emphasize], pch=pch, lwd=lwd,...)
      }
    }
  }
...
plotGroupFeature <- function(cm.mat=top.cm, groups=NULL, gene.column="feature", dr="umap",group.column="group", add.cluster=T, 
                             cluster.col="RNA_snn_res.1", seu.obj=duo, group.prefix="Cluster", do.scale=T, col.num=6, cex=2, cex.main=5,
                             plot.name=NULL, do.par=FALSE, core.num=5, nCols=c("dark green", "yellow", "red"), point.order= "random",
                             return.score=F){
  if(is.null(groups)){
  ...
  if(do.par){
    ...
    score.mat <- foreach(group=groups, .multicombine = T, .combine='cbind')%dopar%{
      ...
      gene <- intersect(cm.mat[[gene.column]][idx], rownames(seu.obj))
      if(do.scale){
        score <- apply(scale(t(as.matrix(expr.mat[gene,]))), 1, sum)
      }else{
        score <- colSums(expr.mat[gene,])
      }
  ...
  for(x in colnames(score.mat)){
    plotFeature2(plot.coor, values=score.mat[,x], main=x, cex.main=cex.main, cex=cex, add.cluster=add.cluster, cl.vec=seu.obj@meta.data[[cluster.col]], label.cex=3, nCols = nCols, point.order= point.order)
  }
  dev.off()
  if(return.score){
    return(score.mat)
...
plotFeature.batch <- function(seu.obj=hio, assay.type="RNA", data.type="data", add.group.info=FALSE, group.by="RNA_snn_res.1", dr="umap", 
                              genes.to.plot=NULL, plot.name=NULL, nCols=c("darkgreen", "yellow","red"), point.order="sorted", do.plot=T,
                              col.num=ifelse(do.plot, 6, 1), label.cex = ifelse(do.plot, 3, 1),  cex.main=ifelse(do.plot, 7, 2), 
                              cex=ifelse(do.plot, 3, 1), per.plot.size=1000, plot.unit="px", plot.res=NA, emphasize=NULL, mar=c(2,7,12,2),
                              with.outline=T, outline.col="#303030", pch=ifelse(with.outline, 21, 16), lwd=0.1, ...){
  ...
  if(do.plot){
    ...
    par(mfrow=c(row.num, col.num), mar=mar)
    for(g in genes.to.plot){
      ...
      plotFeature2(plot.coor, values=slot(seu.obj@assays[[assay.type]], data.type)[g,], main=g, add.cluster=add.group.info, 
                   cl.vec=group.vec, label.cex = label.cex,  cex.main=cex.main, cex=cex, nCols = nCols, 
                   point.order= point.order, emphasize = emphasize, with.outline=with.outline, outline.col=outline.col, pch=pch, lwd=lwd,)	
    }
    dev.off()
  }else{
    par(mfrow=c(row.num, col.num))
...
prepareTreeAndHeatmapInput <- function(expr.mat=hio.expr, cor.method="pearson", hc.method="average", norm.method="quantile", 
                                       genes.to.highlight=NULL, column.reorder=TRUE, row.reorder=TRUE, reorder.method=NULL){
  ...
  }else if(!row.reorder & column.reorder){
    expr.mat <- expr.mat[, col.orders]
    if(is.null(reorder.method)){
      max.ct <- rownames(expr.mat)[apply(expr.mat, 2, which.max)]
      idx <- unlist(lapply(rownames(expr.mat), function(x){
        which(max.ct==x)
      }))
      expr.mat <- expr.mat[,idx]
    }else if (reorder.method=="one2one"){
      new.mat.col.idx <- c()
      remain.col.idx <- colnames(expr.mat)
      for(i in seq(ncol(expr.mat)-1)){
        idx.i <- remain.col.idx[which.max(expr.mat[i,remain.col.idx])]
        new.mat.col.idx <- c(new.mat.col.idx, idx.i)
        remain.col.idx <- setdiff(colnames(expr.mat), new.mat.col.idx)
        if(i==(ncol(expr.mat)-1)){
          new.mat.col.idx <- c(new.mat.col.idx, remain.col.idx)
        }
      }
      expr.mat <- expr.mat[,new.mat.col.idx]
    }
  }
  ...
  res <- list("heatmap_input"=input, "hc_row"=hc.row, "hc_col"=hc.col, 
              "highlighted_genes"=intersect(rownames(normed.expr), genes.to.highlight),
              "highlight_input"=highlight.input)
  return(res)
}
...
getAveExpr <- function(input.type="Seurat", seu.obj, assay.type="RNA", data.type="data", feature.to.calc, 
                       X=NULL, y=NULL, method="mean", specified.order=NULL, size.cutoff=5, genes=NULL, 
                       do.par=TRUE, core.num=5, add.cluster=T, colname.prefix="Cluster", check.barcode=T){
	library(Matrix)
  if(input.type=="matrix"){
    if(sum(colnames(X) != names(y))>0){
...
getExpressedProp <- function(seu.obj,assay.type="RNA", data.type="data", feature.to.calc, expr.cutoff=0, 
                             specified.order=NULL, size.cutoff=10, genes=NULL, core.num=5, add.cluster=T, 
                             colname.prefix="Cluster"){
      library(Matrix)
      meta.data=seu.obj@meta.data
      expr=slot(seu.obj@assays[[assay.type]], data.type)
      if(sum(rownames(meta.data)!=colnames(expr))>0){
     ...
     expr.prop <- foreach(k=sorted.features, .multicombine=T, .combine='cbind')%dopar%{
             ...
             rowSums(expr[genes, sample.idx]>expr.cutoff)/length(sample.idx)
     }
     stopImplicitCluster()
...
plotClusterExprProfile <- function(expr=combined.expr, time.vec=combined.time.points, group.vec=combined.group, cluster.vec=g, 
                                   group.cols=c("#31a354","#3182bd"), return.value=T, to.plot=T, plot.name=NULL, add.legend=T, 
                                   legend.pos="topleft", cex.legend=2, col.num=4, border.do.smooth=F, mean.do.smooth=F, df=8, ...){
    ...
    if(to.plot){
        ...
        for(i in seq(length(g.list))){
            ...
            plot(c(min(time.vec), max(time.vec)), c(ymax, ymin), type="n", xlab="Pseudotime bin",ylab="Relative expr", main=paste("Cluster",i,"(",g.size[i],")"), ...)
            for(group in unique(group.vec)){
                ...
                g.sd <- sd.vec[g.idx]
                if(border.do.smooth){
                  pred.up <- smooth.spline(g.time, g.mean+g.sd, df=df)$y
                  pred.down <- smooth.spline(g.time, g.mean-g.sd, df=df)$y
                  polygon(c(g.time, rev(g.time)), c(pred.up, rev(pred.down)), border=NA, col=paste0(group.cols[group],"30"))
                }else{
            ...
            for(group in unique(group.vec)){
                ...
                g.mean <- mean.vec[g.idx]
                if(mean.do.smooth){
                  lines(g.time, smooth.spline(g.time, g.mean, df=df)$y, col=group.cols[group], lwd=2)
                }else{
...
getMostFrequentGroupByPt <- function(pt.vec=pt.vec, group.vec=age.vec, cell.num.per.bin=50){
  ...
  idx <- cbind(start.idx, end.idx)
  pt.rank <- rank(pt.vec, ties.method = "first")
  most.freq.group <- sapply(seq(nrow(idx)), function(i){
    idx.start <- idx[i,1]
    idx.end <- idx[i,2]
    idx.cell <- which(pt.rank>=idx.start & pt.rank<=idx.end)
    freq <- table(group.vec[idx.cell])
    names(freq)[which.max(freq)]
  })
  return(most.freq.group)
}

run.quadratic.programming <- function(X=X, que.expr=que.expr){
  library(quadprog)
  identity.que <- matrix(NA, nrow=ncol(que.expr), ncol=4)
  rownames(identity.que) <- colnames(que.expr)
  colnames(identity.que) <- c("frxn_state1", "frxn_state2", "Lagrangian", "Error")
